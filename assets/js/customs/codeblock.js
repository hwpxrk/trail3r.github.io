...
(() => {
    ...
    const copy_button_class = "codeblock-copy_button";
    ...
    function create_element(element_name, class_name, text_content = "") {
    ...
    /* Metadata */
    function get_language(codeblock) {
    ...
    function render_metadata(codeblock) {
        const pre = codeblock.querySelector(":scope > pre", );
        const existing_masthead = codeblock.querySelector(":scope > .codeblock-masthead",);

        if (!pre || existing_masthead) return;

        const filename = codeblock.dataset.filename?.trim();
        const masthead = create_element("div", "codeblock-masthead", );
        const metadata = create_element("div", "codeblock-masthead__meta", );

        metadata.append(
            create_element("span", "codeblock-masthead__language", get_language(codeblock), ),
        );

        if (filename) {
            metadata.append(create_element("span", "codeblock-masthead__filename", filename, ), );
        }
    ...
    /* Line numbers */
    function create_gutter(line_count) {
        const gutter = create_element("div", "gutter", );

        for (let line_number = 1; line_number <= line_count; line_number += 1) {
            gutter.append(create_element("span", "", line_number, ));
        }

        return gutter;
    }

    function render_line_numbers(codeblock) {
        let body = codeblock.querySelector(":scope > .codeblock__body", );
        const pre = body ? body.querySelector(":scope > pre", ) : codeblock.querySelector(":scope > pre", );
        const existing_gutter = body?.querySelector(":scope > .gutter", );

        if (!pre || existing_gutter) return;

        if (!body) {
            body = create_element("div", "codeblock__body", );

            pre.before(body);
            body.append(pre);
        }

        pre.before(create_gutter(get_line_count(pre.textContent ?? ""), ));
    }

    /* Rainbow brackets */
    function create_bracket(owner_document, bracket_character, depth = null) {
        const bracket = owner_document.createElement("span");

        bracket.textContent = bracket_character;

        if (depth === null) {
            bracket.className = "bracket-mismatch";
            return bracket;
        }

        const color_depth = ((depth - 1) % bracket_depth_limit) + 1;

        bracket.classList.add("bracket", `bracket-depth-${color_depth}`);
    ...
    function is_processable_text_node(text_node) {
        ...
        const excluded_context = parent.closest(".bracket, .bracket-mismatch, .ts-comment", );
        ...
        const tree_walker = owner_document.createTreeWalker(
            ...
            NodeFilter.SHOW_TEXT,
            {
                acceptNode(text_node) {
                    return is_processable_text_node(text_node) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT;
                },
            },
        );
        const text_nodes = [];
    ...
    function render_rainbow_brackets(code_element) {
        ...
        for (const text_node of text_nodes) {
            ...
            for (const token of tokens) {
                ...
                if (opening_brackets.has(token)) {
                    ...
                    const element = create_bracket(owner_document, token, depth, );
                ...
                if (closing_brackets.has(token)) {
                    ...
                    if (opener?.character === expected_opener) {
                        fragment.append(create_bracket(owner_document, token, opener.depth, ), );
                        bracket_stack.pop();
                    } else {
                        fragment.append(create_bracket(owner_document, token));
                    }
        ...
        for (const opener of bracket_stack) {
            opener.element.className = "bracket-mismatch";
        }
    }
    ...
    function create_copy_icon() {
        const svg_namespace = "http://www.w3.org/2000/svg";
        const icon = document.createElementNS(svg_namespace, "svg");
        const path = document.createElementNS(svg_namespace, "path");

        icon.classList.add("codeblock-copy_button_icon");
    ...
    function update_copy_icon(icon, is_copied) {
        icon.firstElementChild?.setAttribute("d", is_copied ? copied_icon_path : copy_icon_path, );
    }
    ...
    function render_copy_button(codeblock) {
        const masthead = codeblock.querySelector(
            ":scope > .codeblock-masthead",
        );
        ...
        const wrapper = create_element("div", "codeblock-copy", );
        const button = create_element("button", copy_button_class, );
        const icon = create_copy_icon();
        ...
        function update_copy_state(label, is_copied) {
            update_copy_button_label(button, label);
            update_copy_icon(icon, is_copied);
            button.classList.toggle("is-copied", is_copied);

            button.dispatchEvent(
                new CustomEvent("copybutton:state", {
                    bubbles: true,
                    detail: {label, copied: is_copied, },
                }),
            );
        }

        function schedule_copy_state_reset() {
            reset_timer = window.setTimeout(() => {
                reset_timer = null;
                update_copy_state("코드 복사", false);
        ...
        async function copy_code() {
            ...
            try {
                ...
                update_copy_state("복사 완료", true);
            } catch {
                update_copy_state("복사 실패", false);
            } finally {
                is_copying = false;
                schedule_copy_state_reset();
            }
        }

        button.addEventListener("click", copy_code, );
        button.addEventListener("copybutton:request", copy_code, );
    }

    /* Floating copy control */
    ...
    function initialize_floating_copy_button() {
        ...
        const existing_floating_wrapper = document.querySelector(".codeblock-copy--floating", );
        const target_codeblocks = Array.from(codeblocks).filter(is_floating_target, );

        if (!masthead || existing_floating_wrapper || target_codeblocks.length === 0) return;

        const floating_wrapper = create_element(
            "div",
            "codeblock-copy--floating",
        );
        const floating_button = create_element(
            "button",
            "codeblock-copy_button codeblock-copy_button--floating",
        );
        const floating_icon = create_copy_icon();
        ...
        function update_floating_top() {
            const masthead_bottom = masthead.getBoundingClientRect().bottom;

            floating_top = Math.round(Math.max(0, masthead_bottom) + floating_top_gap, );
        }

        function sync_floating_button(original_button) {
            const label = original_button.getAttribute("aria-label");

            if (!label) return false;

            const is_copied = original_button.classList.contains("is-copied");

            update_copy_button_label(floating_button, label);
            update_copy_icon(floating_icon, is_copied);
            floating_button.classList.toggle("is-copied", is_copied);
        ...
        function update_floating_button() {
            ...
            if (next_codeblock !== active_codeblock) {
                ...
                if (
                    !original_button ||
                    !sync_floating_button(original_button)
                ) {
                    hide_floating_button();
                    return;
                }
            ...
            const floating_right = Math.max(viewport_edge_gap, Math.round(window.innerWidth - codeblock_right + floating_right_gap, ), );
        ...
        document.addEventListener("copybutton:state", (event) => {
